Create Codex pet row `running-right` for `xiaogui`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A small mechanical hamster with brass gear accents, tiny orange-tinted goggles on its forehead, a small wrench prop in one paw, cute round body with visible gear details on its back, standing pose, friendly and industrious expression. Preserve silhouette, face, palette, material, proportions, markings, and props.
