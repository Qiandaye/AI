Create one horizontal animation strip for Codex pet `xiaogui`, state `running-right`.
...
Identity: same pet in every frame: A small mechanical hamster with brass gear accents, tiny orange-tinted goggles on its forehead, a small wrench prop in one paw, cute round body with visible gear details on its back, standing pose, friendly and industrious expression. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `3d-toy`: Stylized 3D toy mascot with smooth rounded forms, simple materials, clear silhouette, and no photoreal complexity. User style notes: Cute miniature mechanical toy style, warm brass and copper metallic tones, soft ambient lighting, clean readable silhouette at pet size.
